...
- 🔴 ОДИН БЛОК — ОБА ЯЗЫКА: ru и uz это два ПОЛЯ одного блока, а не два разных блока. Язык переключается тумблером на схеме, дублировать схему НЕ НАДО. Пары «Приветствие (рус)» + «Приветствие (уз)», отдельные русская и узбекская ветки — ЗАПРЕЩЕНЫ. Пометки языка в title («(рус)», «(уз)», «(uz)») — тоже запрещены.
...
🔴 ЖИВОЙ ПРОДАЮЩИЙ ТОН (это самое важное для качества):
- Пиши как живой человек говорит по телефону, а не как пишут в документе. Реплики должны звучать естественно вслух.
- Каждая реплика при обработке возражения должна ВЕСТИ К ЦЕЛИ: не просто ответить, а мягко вернуть клиента к нужному действию. Заканчивай зацепкой, вопросом или выгодой, а не сухой констатацией.
- ЗАПРЕЩЕН канцелярит и штампы: «предусмотрены выгодные условия», «можно узнать у специалиста», «осуществляется обслуживание», «в целях информирования». Это мёртвые фразы — переписывай их живой речью.
- НЕ повторяй одну и ту же пустую фразу в разных блоках («наши кредиты помогут без лишних затрат» в пяти блоках подряд — это провал). Каждый блок отвечает на СВОЁ возражение конкретно и по-разному.
- Говори про выгоду клиента конкретно (что он получит), а не абстрактно («это удобно», «выгодные условия» — пустышки).
- Работая с возражением: сначала короткое присоединение («понимаю»), потом аргумент по сути возражения, потом мягкий возврат к цели. Не отписка в одну строку.

🔴 УСТНАЯ РЕЧЬ, А НЕ ЛИТЕРАТУРА (реплику произносят вслух, её слушают, а не читают):
- ЗАПРЕЩЕНЫ кальки с английского: «Как вы себя чувствуете с этим долгом?», «Мы здесь, чтобы помочь вам», «Я представляю банк», «Будьте уверены», «Не стесняйтесь обращаться».
- ЗАПРЕЩЕНЫ книжные слова: «данный», «указанный», «настоящий», «осуществить», «произвести оплату», «в целях», «при необходимости», «в случае возникновения».
- Отглагольные существительные заменяй глаголом: «обсудить возможность реструктуризации» → «пересобрать график», «для осуществления оплаты» → «чтобы оплатить», «снизить долговую нагрузку» → «платить меньше».
- ЗАПРЕЩЕНА литературная вода без факта: «Это действительно важно», «Нельзя упускать это из виду», «Это может быть сложно», «Мы ценим вас как клиента».
- Примеры переписывания: «Я звоню, чтобы обсудить возможность реструктуризации вашего долга. У вас есть время?» → «Звоню по вашей рассрочке — можем уменьшить платёж. Минута есть?»; «Ваш текущий долг составляет {AMOUNT}. Как вы себя чувствуете с этим долгом?» → «По договору за вами {AMOUNT}. Получится закрыть в этом месяце?».
- Предложения короткие — 5-12 слов, обычные разговорные слова. Проверь себя: если так не скажут вслух по телефону — переписывай.
...
Тексты — живые, человечные, продающие, в тоне "{tone}", сразу оба языка (ru и uz). Каждая реплика по возражению ведёт клиента к цели, а не просто отвечает. Никакого канцелярита и повторов одной фразы. Если задан эталон — бери у него ТОЛЬКО структуру, тексты пиши с нуля под эту тему.
...
Возвращаешь СТРОГО JSON: {"score":7,"summary":"1-2 предложения: общий вывод и главный приоритет","to_next_score":"что исправить, чтобы поднять оценку на 1-2 балла (2-3 пункта одной фразой)","strengths":["сильная сторона 1","сильная сторона 2"],"sections":[{"name":"Название раздела","assessment":"1-2 предложения: как раздел работает на цель и что улучшить","issues":[{"severity":"high|medium|low","blockId":"id блока или пусто","type":"конверсия|возражение|тон|смысл|узбекский|длина|логика","message":"в чём проблема","before":"проблемная фраза как есть","suggestion":"переписанный вариант","reason":"почему так лучше — 1 предложение"}]}]}. score — целое 1-10 по ЧЁТКОЙ ШКАЛЕ (используй весь диапазон, не занижай по привычке): 10 — эталон, готов в продакшен без единой правки, придраться не к чему; 9 — отличный, лишь мелкие стилистические придирки, все критичные и важные вещи закрыты; 8 — сильный, есть 2-3 некритичных улучшения; 7 — рабочий, но есть заметные улучшения по тексту/тону; 5-6 — есть важные проблемы (сухой тон, канцелярит, слабая работа с возражениями); 3-4 — много проблем, скрипт сырой; 1-2 — не годится. ВАЖНО: если у скрипта нет ни одной high/medium проблемы — ставь 9 или 10, НЕ занижай до 7 «на всякий случай». Оценка должна отражать реальное качество: отличный скрипт ДОЛЖЕН получать 9-10. Только сам JSON-объект, без markdown-обёртки из тройных обратных кавычек.`,
...
function upgradeStalePrompts() {
  ...
  if (!/ОДИН БЛОК — ОБА ЯЗЫКА/.test(gs)) {
...
async function _doImproveBlock(b, mode, currentRu, currentUz, uiMode) {
  ...
  try {
    ...
    if (uiMode === 'canvas') {
      ...
      b.ru = parsed.ru;
      b.uz = parsed.uz;
      // Текст написал AI, но ПРИНЯЛ его пользователь — это его выбор, и он
      // такой же сигнал предпочтения, как правка руками. Базовую линию НЕ
      // трогаем: captureEditForLearning увидит расхождение, запишет пару
      // «было → принятое» и сдвинет линию только после подтверждения записи.
      captureEditForLearning(b);
...
function applyAISuggestion(blockId, newRu, newUz) {
  ...
  b.ru = newRu;
  b.uz = newUz;
  // Пользователь нажал «Применить» — принятая подсказка идёт в обучение как
  // его выбор (см. _doImproveBlock).
  captureEditForLearning(b);
...
function captureEditForLearning(b, prevRu, prevUz) {
  try {
    ...
    // Приняли подсказку AI и сразу нажали «Сохранить» — capture срабатывает
    // дважды, а базовая линия двигается только по ответу облака, поэтому вторая
    // попытка успевает записать ту же пару. Гасим повтор по блоку и тексту.
    const flightKey = `${b.id}|${b.ru || ''}|${b.uz || ''}|${b.title || ''}`;
    if (_editInFlight.has(flightKey)) return;
    _editInFlight.add(flightKey);
    ...
    Promise.resolve(cloudSaveEdit({
      ...
      .finally(() => _editInFlight.delete(flightKey));
...
async function buildLearningSection(quiet) {
  ...
  try {
    ...
    const pairs = picked
      ...
      .join('\n\n');
    // Узбекские правки раньше терялись: запись в базу шла, но пары собирались
    // ТОЛЬКО из ru, и правка одного лишь uz превращалась в «негодную пару».
    const uzEdits = edits.filter(e => e.final_uz && e.ai_uz && e.final_uz !== e.ai_uz);
    const uzPairs = uzEdits
      .slice(0, 6)
      .map(e => `Было (AI): ${e.ai_uz}\nСтало (правка пользователя): ${e.final_uz}`)
      .join('\n\n');
    ...
    const titlePairs = titleEdits
      ...
      .join('\n');
    say(`[Cybernet] обучение на правках: в базе последних ${edits.length}, пар по русскому ${usable.length} (в промпт ${picked.length}), по узбекскому ${uzEdits.length}, переименований ${titleEdits.length}`);
    if (!pairs && !uzPairs && !titlePairs) {
      _learningCache = { at: Date.now(), text: '' };
      return '';
    }
    let text = '';
    ...
    if (uzPairs) {
      text += '\n\n=== КАК ЭТОТ ПОЛЬЗОВАТЕЛЬ ПРАВИТ УЗБЕКСКИЕ ТЕКСТЫ (та же манера, только латиница) ===\n' + uzPairs;
    }
    if (titlePairs) {
...
async function testLearning(blockId, probe) {
  ...
  const rows = {
    ...
    // Формулировка «ничего не изменилось» пугала: её видно и сразу ПОСЛЕ
    // успешной записи, когда базовая линия уже сдвинулась на новый текст.
    // Это нормальное состояние, а не отказ.
    'ИТОГ': (ruChanged || uzChanged || titleChanged)
      ? 'есть что записать — сохраните блок'
      : 'записывать нечего: текущий текст уже принят за базовый (либо правки не было, либо она уже записана)'
  };
  console.table(rows);
  ...
  if (hasBase) {
    ...
    if (!ruChanged && !uzChanged && !titleChanged) {
      console.warn('[Cybernet] отличий нет — текущий текст уже базовый. Если вы только что видели «✓ правка сохранена», это нормально: пара записана, линия сдвинута.');
    }
  }
...
- Бери из эталонов СТРУКТУРУ и НАБОР ИНТЕНТОВ, а не готовые формулировки
- 🔴 В готовых текстах НЕ должно остаться НИ СЛОВА из тем эталонов — только новая тема из запроса
...
3) Если база описывает сценарий короче, чем запрошенный размер скрипта — дополни его обработкой особых ситуаций и возражений, НЕ ломая описанную логику.` : '';
  ...
  const hasOwnScenario = (extras && extras.trim().length > 200) || (knowledge && knowledge.trim().length > 200);
  if (hasOwnScenario) {
...
- 🔴 ОДИН БЛОК — ОБА ЯЗЫКА. ru и uz это ДВА ПОЛЯ ОДНОГО блока, а НЕ два разных блока. В приложении язык переключается тумблером прямо на схеме, поэтому схему НЕ НАДО дублировать.
  ❌ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО делать пары блоков на два языка: «Приветствие (рус)» + «Приветствие (уз)», «Кто вы? (рус)» + «Kim siz? (uz)», отдельная русская ветка и отдельная узбекская. Это удваивает схему и делает её нечитаемой.
  ❌ ЗАПРЕЩЕНО писать язык в title: «(рус)», «(уз)», «(РУС)», «(uz)», «(uz, 2 marta)». Заголовок ОДИН, по-русски, без языковых пометок.
  ❌ ЗАПРЕЩЕНО оставлять блок с заполненным ru и пустым uz (или наоборот). У КАЖДОГО речевого блока заполнены ОБА поля.
  ✅ Правильно: {"title":"Приветствие","ru":"Здравствуйте…","uz":"Assalomu alaykum…"} — один блок, два поля.
  ✅ Если делаешь ромб «Проверка языка» — ОБЕ его ветки ведут в ОДИН И ТОТ ЖЕ следующий блок, потому что у того блока уже есть оба текста:
  {"id":"check_lang","title":"Проверка языка","type":"decision","ru":"","uz":"","branches":[{"label":"Рус","next":"greeting"},{"label":"Уз","next":"greeting"}]}
- 🔴 РОМБЫ type:"decision" — ОБЯЗАТЕЛЬНЫ. Ромб = ТЕХНИЧЕСКАЯ ПРОВЕРКА без речи робота (ru и uz пустые!), из него выходят ветки с вариантами.
...
- 🔴 ЗАГОЛОВКИ БЛОКОВ (title) И МЕТКИ ВЕТОК — ТОЛЬКО ПО-РУССКИ. «Приветствие», «Нет денег», «Кто вы?», «Согласие», «Отказ» — а НЕ «Greeting», «No Money Objection», «Refusal». Английские заголовки — ошибка.`;
  ...
  // The typical call-centre skeleton used to be part of the block above and was
  // appended unconditionally — so a user who had written their own scenario got
  // the same hardcoded «Проверка языка → Это клиент? → Какой раз повторяет?»
  // every time, with the exception handlers bolted on, regardless of what they
  // actually asked for. Keep it as the DEFAULT plan, and demote it to a
  // fallback as soon as the user has described their own flow.
  if (hasOwnScenario) {
    userPrompt += `\n\n📋 ТИПОВЫЕ ЭЛЕМЕНТЫ (только как подсказка, НЕ как обязательный план):
Сценарий пользователя выше — главный. Ромбы «Проверка языка», «Это клиент?», «Какой раз повторяет?» и блоки особых ситуаций («не слышно», «вы робот?», «мошенники», «перезвоните позже») добавляй ТОЛЬКО если они есть в его сценарии или он этот участок не описал. Не подменяй его шаги этим набором.`;
  } else {
    userPrompt += `\n\n📋 ТИПОВОЙ ПЛАН СКРИПТА (пользователь свой сценарий не описал — используй этот):
- Ромбы: «Проверка языка» (ветки «Рус» / «Уз» — обе в ОДИН блок), «Это клиент?» (ветки «Да» / «Нет»), «Какой раз повторяет?» (ветки «1ый раз» / «2ой раз» / «3ий раз»).
...
- role «Посоветуюсь / подумаю» → робот отвечает: «Конечно, решение важное. Давайте я коротко скажу главное, чтобы вам было с чем идти к партнёрам…»
- НЕПРАВИЛЬНО: «Дайте мне время подумать» ← снова слова клиента.
Проверяй каждую реплику: её произносит РОБОТ-менеджер банка, который ведёт клиента к цели.

🔴 РОЛЬ СФОРМУЛИРОВАНА СЛОВАМИ СТАРОЙ ТЕМЫ:
role может содержать предмет эталона («У нас уже есть 1С», «Что входит в тестовый период»). Пойми, какое это ВОЗРАЖЕНИЕ по смыслу, и переложи на НОВУЮ СФЕРУ:
...
- 🔴 УСТНАЯ РЕЧЬ, А НЕ ЛИТЕРАТУРА (реплику произносят ВСЛУХ, её слушают, а не читают):
  ❌ Кальки с английского: «Как вы себя чувствуете с этим долгом?», «Мы здесь, чтобы помочь вам», «Я представляю банк», «Будьте уверены», «Не стесняйтесь обращаться».
  ❌ Канцелярит и книжные слова: «данный», «указанный», «настоящий», «осуществить», «произвести оплату», «в целях», «при необходимости», «в случае возникновения».
  ❌ Отглагольные существительные — ставь ГЛАГОЛ: «обсудить возможность реструктуризации» → «пересобрать график», «для осуществления оплаты» → «чтобы оплатить», «снизить долговую нагрузку» → «платить меньше».
  ❌ Литературная вода без факта: «Это действительно важно», «Нельзя упускать это из виду», «Это может быть сложно», «Это может быть запутанно», «Мы ценим вас как клиента».
  ✅ Как надо — те же мысли, переписанные вслух:
     «Я звоню, чтобы обсудить возможность реструктуризации вашего долга. У вас есть время для разговора?» → «Звоню по вашей рассрочке — можем уменьшить платёж. Минута есть?»
     «Ваш текущий долг составляет {AMOUNT}. Как вы себя чувствуете с этим долгом?» → «По договору за вами {AMOUNT}. Получится закрыть в этом месяце?»
     «Понимаю, это может быть сложно. Мы предлагаем возможность реструктуризировать данный долг.» → «Понимаю. Тогда разобьём сумму на месяцы — платёж станет меньше.»
     «Мы здесь, чтобы помочь вам.» → «Давайте подберём срок, который вы потянете.»
     «Я представляю банк. Если у вас есть сомнения, свяжитесь с нами напрямую.» → «Я из {BANK_NAME}, звоню по вашему договору. Можете перепроверить номер в приложении.»
  ✅ Предложения короткие — 5-12 слов. Обычные разговорные слова. Проверь себя: если так не скажут вслух коллеге по телефону — переписывай.
...
${JSON.stringify(mapChunk, null, 1)}`;
      ...
      try {
        ...
        const stillBookish = vals.filter(r => literaryHits(r) > 0).length;
        console.log(`[Cybernet] качество реплик: с цифрами ${withNum}/${vals.length}, пустышек ${withFiller}, книжных ${stillBookish} (было ${bookish}), средняя длина ${avgWords} слов, разных фактов из базы ${distinctFacts}`);
      } catch (e) {}
      ...
      const newProfile = {
        ...
        blocks: refBlocks.map(b => {
          ...
          // Structural labels («Рус», «Уз», «Да», «1ый раз») are part of the
          // flow, not copy. The model renamed «Рус» to «Выбор языка» and then
          // wrote a language question into it — re-asking what the incoming
          // branch had just answered. Pin these titles in code.
          const refTitle = (b.title || '').trim();
          const isStructural = /^(рус|уз|ru|uz)$/i.test(refTitle)
            || /^(да|нет|yes|no)[\s,!.]*$/i.test(refTitle)
          ...
          return {
            ...
            branches: (b.branches || []).map(br => ({
              ...
              // the new script had zero edge geometry and draw.io auto-routed
              // every arrow from scratch — that's the "стрелки хаосные" bug.
              _dio: br._dio,
              waypoints: br.waypoints ? br.waypoints.map(p => ({ x: p.x, y: p.y })) : undefined,
              exitX: br.exitX, exitY: br.exitY, exitDx: br.exitDx, exitDy: br.exitDy,
              entryX: br.entryX, entryY: br.entryY, entryDx: br.entryDx, entryDy: br.entryDy,
    ...
    // Языковое раздвоение схемы. Вместо одного блока с полями ru+uz модель
    // строит две параллельные подсхемы — «Приветствие (рус)» и «Приветствие
    // (уз)» — и схема раздувается вдвое (замер на реальной генерации: 71 блок,
    // из них с обоими языками ровно 1). Правило в промпте теперь есть, но если
    // оно не сработало, лучше сказать прямо, чем молча отдать двойную схему.
    // \b с кириллицей в JS не работает — граница задана явным классом.
    try {
      const speech = parsed.blocks.filter(b => b && b.type !== 'decision' && b.type !== 'start');
      const oneLang = speech.filter(b => {
        const ru = String(b.ru || '').trim(), uz = String(b.uz || '').trim();
        return (ru && !uz) || (!ru && uz);
      }).length;
      const langTitles = parsed.blocks
        .filter(b => /\((?:рус|уз|uz|ru)(?![а-яёa-z])[^)]*\)/i.test(String(b.title || ''))).length;
      if (speech.length && (oneLang / speech.length > 0.3 || langTitles > 2)) {
        console.warn(`[Cybernet] СХЕМА РАЗДВОЕНА ПО ЯЗЫКАМ: одноязычных блоков ${oneLang}/${speech.length}, заголовков с пометкой языка ${langTitles}. Модель сделала отдельные русскую и узбекскую ветки вместо одного блока с ru+uz — схема вдвое больше нужного.`);
        try { toast('Модель раздвоила схему на два языка — стоит перегенерировать'); } catch (e) {}
      } else {
        console.log(`[Cybernet] языки: одноязычных блоков ${oneLang}/${speech.length}, заголовков с пометкой языка ${langTitles}`);
      }
    } catch (e) {}
...
async function fixIssueFromReview(idx) {
  ...
  try {
    ...
    // Нажатие «Исправить» — осознанный выбор пользователя, значит пара «было →
    // принятое» идёт в обучение (см. _doImproveBlock).
    captureEditForLearning(b);
    saveToStorage();
    if (typeof renderBlocks === 'function') renderBlocks();
    if (typeof canvasRender === 'function') canvasRender();
